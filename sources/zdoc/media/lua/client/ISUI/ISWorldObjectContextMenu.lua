...
local function predicateNotFullModItem(item)
	return item:getUsedDelta() < 1 and item:getType() ~= "CarBattery1" and item:getType() ~= "CarBattery2" and item:getType() ~= "CarBattery3"
end
...
--            if sprite and sprite:getParentSprite() and sprite:getParentSprite():getProperties():Is(IsoFlagType.canBeCut) then
	...
	if v:getPipedFuelAmount() > 0 and (playerInv:containsEvalRecurse(predicateEmptyPetrol) or playerInv:containsEvalRecurse(predicatePetrolNotFull)) then
		haveFuel = v;
	end
...
--    print('reused ',#ISWorldObjectContextMenu.tooltipsUsed,' world tooltips')
	...
	if haveFuel and getCore():getDebug() then
		local pumpInfo = context:addOption(getText("ContextMenu_PumpFuelAmount") .. ": " .. tostring(haveFuel:getPipedFuelAmount()))
		pumpInfo.iconTexture = getTexture("media/ui/BugIcon.png")
		-- context:addOption(getText("ContextMenu_PumpFuelAmount") .. ":" .. tostring(v:getPipedFuelAmount()) ), {heavyItem}, ISInventoryPaneContextMenu.onUnEquip, player)
	end
...
--    end
	...
	if waterDispenser and playerObj:DistToSquared(waterDispenser:getX() + 0.5, waterDispenser:getY() + 0.5) < 2 * 2 then
		...
		if waterDispenser:isTaintedWater() and getSandboxOptions():getOptionByName("EnableTaintedWaterText"):getValue() then
	...
	if storeWater then
		if not clothingDryer and not clothingWasher and not comboWasherDryer then --Stops being able to wash clothes in washing machines and dryers
	...
	if ISWorldObjectContextMenu.toggleComboWasherDryer(context, playerObj, comboWasherDryer) then
		return true
	end
...
--    context:addOption("Check Stats2", worldobjects, ISWorldObjectContextMenu.onCheckStats, playerObj, playerObj)
    ...
    if generator then
        ...
        if not generator:isActivated() and generator:getFuel() < 100 and playerInv:containsEvalRecurse(predicatePetrol) then
...
function ISWorldObjectContextMenu.handleInteraction(x, y, test, context, worldobjects, playerObj, playerInv)
	...
	for name,items in pairs(itemList) do
		...
		if instanceof(_item, "Radio")  then
			...
			if _obj ~= nil then
				context:addOption(getText("IGUI_DeviceOptions"), playerObj, function(pl, obj) ISRadioWindow.activate(pl, obj, true) end, _obj );
			end
		end
	end
end
...
function ISWorldObjectContextMenu.handleCarBatteryCharger(test, context, worldobjects, playerObj, playerInv)
	...
	if carBatteryCharger:getBattery() then
		...
		playerInv:getAllTagEvalRecurse("CarBattery", predicateNotFullModItem, batteryList)
...
function ISWorldObjectContextMenu.toggleComboWasherDryer(context, playerObj, object)
	...
	local option = nil
	if object:isActivated() then
		option = subMenu:addOption(getText("ContextMenu_Turn_Off"), playerObj, ISWorldObjectContextMenu.onToggleComboWasherDryer, object)
	else
		option = subMenu:addOption(getText("ContextMenu_Turn_On"), playerObj, ISWorldObjectContextMenu.onToggleComboWasherDryer, object)
	end
	...
	option = subMenu:addOption(label, playerObj, ISWorldObjectContextMenu.onSetComboWasherDryerMode, object, object:isModeWasher() and "dryer" or "washer")
end
...
ISWorldObjectContextMenu.doFillWaterMenu = function(sink, playerNum, context)
	...
	if pourInto:size() > 1 then
		containerOption = containerMenu:addOption(getText("ContextMenu_FillAll"), worldobjects, ISWorldObjectContextMenu.onTakeWater, sink, allContainers, nil, playerNum);
	...
	for _,containerType in pairs(allContainerTypes) do
		...
		if #containerType > 1 then --#containerType gets the length of the table.
			...
			containerMenu:addSubMenu(containerOption, containerTypeMenu)
			local containerTypeOption
			containerTypeOption = containerTypeMenu:addOption(getText("ContextMenu_FillOne"), worldobjects, ISWorldObjectContextMenu.onTakeWater, sink, nil, destItem, playerNum);
			containerTypeOption.toolTip = tooltip
			if containerType[2] ~= nil then
				containerTypeOption = containerTypeMenu:addOption(getText("ContextMenu_FillAll"), worldobjects, ISWorldObjectContextMenu.onTakeWater, sink, containerType, nil, playerNum);
				containerTypeOption.toolTip = tooltip
			end
		else
			...
			if instanceof(destItem, "DrainableComboItem") then
				t.description = t.description .. " <LINE> " .. getText("ContextMenu_ItemWaterCapacity") .. ": " .. math.floor(destItem:getUsedDelta()*10) .. "/10"
			end
			containerOption.toolTip = t
		end
	end
...
ISWorldObjectContextMenu.doFillFuelMenu = function(source, playerNum, context)
	...
	context:addSubMenu(fillOption, containerMenu)
	if pourInto:size() > 1 then
		containerOption = containerMenu:addOption(getText("ContextMenu_FillAll"), worldobjects, ISWorldObjectContextMenu.onTakeFuelNew, source, allContainers, nil, playerNum);
	end
	--add the fill container of type menu
	for _,containerType in pairs(allContainerTypes) do
		local destItem = containerType[1]
		if #containerType > 1 then --#containerType gets the length of the table.
			...
			containerMenu:addSubMenu(containerOption, containerTypeMenu)
			local containerTypeOption
			containerTypeOption = containerTypeMenu:addOption(getText("ContextMenu_FillOne"), worldobjects, ISWorldObjectContextMenu.onTakeFuelNew, source, nil, destItem, playerNum);
			if containerType[2] ~= nil then
				containerTypeOption = containerTypeMenu:addOption(getText("ContextMenu_FillAll"), worldobjects, ISWorldObjectContextMenu.onTakeFuelNew, source, containerType, nil, playerNum);
			end
		else
			containerOption = containerMenu:addOption(destItem:getName(), worldobjects, ISWorldObjectContextMenu.onTakeFuelNew, source, nil, destItem, playerNum);
			if instanceof(destItem, "DrainableComboItem") then
				local t = ISWorldObjectContextMenu.addToolTip()
				t.maxLineWidth = 512
				t.description = getText("ContextMenu_FuelCapacity") .. string.format("%d / %d", destItem:getDrainableUsesInt(), 1.0 / destItem:getUseDelta() + 0.0001)
				containerOption.toolTip = t
			end
		end
	end
...
ISWorldObjectContextMenu.onTakeFuelNew = function(worldobjects, fuelObject, fuelContainerList, fuelContainer, player)
	...
	for i,item in ipairs(fuelContainerList) do
		-- first case, fill an empty bottle
		if predicateEmptyPetrol(item) then
		
			-- we create the item which contain our petrol
			local newItemType = item:getReplaceType("PetrolSource");
			local newItem = InventoryItemFactory.CreateItem(newItemType,0);
			newItem:setCondition(item:getCondition());
			newItem:setFavorite(item:isFavorite());
			...
			ISTimedActionQueue.add(ISTakeFuel:new(playerObj, fuelObject, item, 100))
			if returnToContainer and (returnToContainer ~= playerInv) and newItem then
				ISTimedActionQueue.add(ISInventoryTransferAction:new(playerObj, newItem, playerInv, returnToContainer))
			end
		elseif predicatePetrolNotFull(item) then  -- second case, a bottle contain some fuel, we just fill it
			if not didWalk and (not fuelObject:getSquare() or not luautils.walkAdj(playerObj, fuelObject:getSquare(), true)) then
				return
			end
			didWalk = true
