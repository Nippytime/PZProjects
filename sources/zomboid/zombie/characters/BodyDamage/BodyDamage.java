...
public final class BodyDamage {
   ...
   public boolean AddRandomDamageFromZombie(IsoZombie var1, String var2) {
      ...
      if (GameServer.bServer) {
         ...
         var5 -= (var11 - 1) * 10;
         int var19 = var6 - (var11 - 1) * 30;
         int var20 = var7 - (var11 - 1) * 15;
         byte var12 = 3;
         ...
         if (!var1.bCrawling) {
            if (Rand.Next(10) == 0) {
               var18 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L), BodyPartType.ToIndex(BodyPartType.Groin) + 1);
            } else {
               var18 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L), BodyPartType.ToIndex(BodyPartType.Neck) + 1);
            }
            ...
            if (var9 && (float)Rand.Next(100) < var13) {
               var18 = BodyPartType.ToIndex(BodyPartType.Neck);
            }

            if (var18 == BodyPartType.ToIndex(BodyPartType.Head) || var18 == BodyPartType.ToIndex(BodyPartType.Neck)) {
               byte var14 = 70;
               if (var9) {
                  var14 = 90;
               }
               ...
               if (Rand.Next(100) > var14) {
                  ...
                  while(true) {
                     do {
                        ...
                        var15 = true;
                        var18 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Torso_Lower) + 1);
                     } while(var18 != BodyPartType.ToIndex(BodyPartType.Head) && var18 != BodyPartType.ToIndex(BodyPartType.Neck) && var18 != BodyPartType.ToIndex(BodyPartType.Groin));
            ...
            if (Rand.Next(10) == 0) {
               var18 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Groin), BodyPartType.ToIndex(BodyPartType.MAX));
            } else {
               var18 = Rand.Next(BodyPartType.ToIndex(BodyPartType.UpperLeg_L), BodyPartType.ToIndex(BodyPartType.MAX));
            }
         }
         ...
         if (GameServer.bServer && this.ParentChar instanceof IsoPlayer || Core.bDebug && this.ParentChar instanceof IsoPlayer) {
            DebugLog.log(DebugType.Combat, "zombie did " + var13 + " dmg to " + ((IsoPlayer)this.ParentChar).getDisplayName() + " on body part " + BodyPartType.getDisplayName(BodyPartType.FromIndex(var18)));
         }
         ...
         if (Rand.Next(100) > var5) {
            var1.scratch = true;
            this.getParentChar().helmetFall(var18 == BodyPartType.ToIndex(BodyPartType.Neck) || var18 == BodyPartType.ToIndex(BodyPartType.Head));
            if (Rand.Next(100) > var20) {
               var1.scratch = false;
            ...
            boolean var17;
            if (var1.scratch) {
               var16 = this.getParentChar().getBodyPartClothingDefense(var18, false, false);
               ...
               if ((float)Rand.Next(100) < var16) {
                  this.getParentChar().addHoleFromZombieAttacks(BloodBodyPartType.FromIndex(var18), var15);
                  return false;
               }

               var17 = this.getParentChar().addHole(BloodBodyPartType.FromIndex(var18), true);
               if (var17) {
                  this.getParentChar().getEmitter().playSoundImpl("ZombieRipClothing", (IsoObject)null);
               }

               var21 = true;
               this.AddDamage(var18, var13);
               this.SetScratched(var18, true);
               this.getParentChar().addBlood(BloodBodyPartType.FromIndex(var18), true, false, true);
               var3 = 1;
               ...
               var16 = this.getParentChar().getBodyPartClothingDefense(var18, true, false);
               ...
               var21 = true;
               this.AddDamage(var18, var13);
               this.SetBitten(var18, true);
               if (var18 == BodyPartType.ToIndex(BodyPartType.Neck)) {
                  this.getParentChar().addBlood(BloodBodyPartType.FromIndex(var18), false, true, true);
                  this.getParentChar().addBlood(BloodBodyPartType.FromIndex(var18), false, true, true);
         ...
         case 0:
            var10000 = this.ParentChar.getStats();
            var10000.Pain += this.getInitialThumpPain() * BodyPartType.getPainModifyer(var18);
            break;
         case 1:
            var10000 = this.ParentChar.getStats();
            var10000.Pain += this.getInitialScratchPain() * BodyPartType.getPainModifyer(var18);
            break;
         case 2:
            var10000 = this.ParentChar.getStats();
            var10000.Pain += this.getInitialBitePain() * BodyPartType.getPainModifyer(var18);
         }
         ...
         if (this.ParentChar instanceof IsoPlayer && GameClient.bClient && ((IsoPlayer)this.ParentChar).isLocalPlayer()) {
            IsoPlayer var22 = (IsoPlayer)this.ParentChar;
            var22.updateMovementRates();
            GameClient.sendPlayerInjuries(var22);
            GameClient.sendPlayerDamage(var22);
         }
   ...
   public void Update() {
      if (!(this.getParentChar() instanceof IsoZombie)) {
         ...
         if (GameServer.bServer) {
            this.RestoreToFullHealth();
            var12 = ((IsoPlayer)this.getParentChar()).bleedingLevel;
            if (var12 > 0) {
               var14 = 1.0F / (float)var12 * 200.0F * GameTime.instance.getInvMultiplier();
               if ((float)Rand.Next((int)var14) < var14 * 0.3F) {
                  this.getParentChar().splatBloodFloor();
               }

               if (Rand.Next((int)var14) == 0) {
                  this.getParentChar().splatBloodFloor();
               }
            }
            ...
            if (this.getOverallBodyHealth() != 0.0F) {
               ...
               if (this.PoisonLevel > 0.0F) {
                  if (this.PoisonLevel > 10.0F && this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Sick) >= 1) {
                     var15 = 0.0035F * Math.min(this.PoisonLevel / 10.0F, 3.0F) * GameTime.instance.getMultiplier();
                     var3 += var15;
                  }
               ...
               if (var15 > 0.0F) {
                  LuaEventManager.triggerEvent("OnPlayerGetDamage", var17, "POISON", var15);
               }

               if (var5 > 0.0F) {
                  LuaEventManager.triggerEvent("OnPlayerGetDamage", var17, "HUNGRY", var5);
               }

               if (var6 > 0.0F) {
                  LuaEventManager.triggerEvent("OnPlayerGetDamage", var17, "SICK", var6);
               }

               if (var7 > 0.0F) {
                  LuaEventManager.triggerEvent("OnPlayerGetDamage", var17, "BLEEDING", var7);
               }

               if (var8 > 0.0F) {
                  LuaEventManager.triggerEvent("OnPlayerGetDamage", var17, "THIRST", var8);
               }

               if (var9 > 0.0F) {
                  LuaEventManager.triggerEvent("OnPlayerGetDamage", var17, "HEAVYLOAD", var9);
               }
               ...
               if (this.ParentChar.getPainEffect() > 0.0F) {
                  ...
                  for(int var16 = 0; var16 < BodyPartType.ToIndex(BodyPartType.MAX); ++var16) {
                     var3 += ((BodyPart)this.getBodyParts().get(var16)).getPain() * BodyPartType.getPainModifyer(var16);
                  }
               ...
               if (this.isInfected()) {
                  ...
                  if (var13 == 1) {
                     ...
                     var15 = this.getCurrentTimeForInfection();
                     ...
                     if (this.InfectionTime < 0.0F) {
                        this.InfectionTime = var15;
                     }

                     if (this.InfectionTime > var15) {
                        this.InfectionTime = var15;
                     }
